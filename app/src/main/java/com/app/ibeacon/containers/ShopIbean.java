package com.app.ibeacon.containers;
...
import com.app.ibeacon.util.CsvWriterHelper;
import com.app.ibeacon.util.TimeFormatter;
 ...
 * @更新描述 by ${todo}
 */
public class ShopIbean{
    public static Map<String, BluetoothLeDevice> mALLDeviceMap;


    public    ShopIbean() {
            mALLDeviceMap = new HashMap<>();

    }


    public void  addDevice(final BluetoothLeDevice device) {
        if (mALLDeviceMap.containsKey(device.getAddress())) {
            mALLDeviceMap.get(device.getAddress()).updateRssiReading(device.getTimestamp(), device.getRssi());
        } else {
            mALLDeviceMap.put(device.getAddress(), device);
        }
    }
    ...
    public List<BluetoothLeDevice> getDeviceList() {
        final List<BluetoothLeDevice> methodResult = new ArrayList<>(mALLDeviceMap.values());
